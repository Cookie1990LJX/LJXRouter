...
//  LJXRouter_Tests
//
//  Created by 北京市吕俊学 on 2018/4/9.
//  Copyright © 2018年 373733319@qq.com. All rights reserved.
//

#import "LJXRouter.h"
#import <objc/runtime.h>

static NSString * const LJX_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const LJXRouterParameterURL = @"LJXRouterParameterURL";
NSString *const LJXRouterParameterCompletion = @"LJXRouterParameterCompletion";
NSString *const LJXRouterParameterUserInfo = @"LJXRouterParameterUserInfo";
...
@implementation LJXRouter

+ (instancetype)sharedInstance
{
    static LJXRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(LJXRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        LJXRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[LJXRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[LJXRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    LJXRouter *router = [LJXRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL];
    LJXRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(LJXRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(LJXRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(LJXRouterObjectHandler)handler
{
    NSMutableDictionary * subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    parameters[LJXRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:LJX_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if ((pathSegments.count == 2 && ((NSString *)pathSegments[1]).length) || pathSegments.count < 2) {
            [pathComponents addObject:LJX_ROUTER_WILDCARD_CHARACTER];
        }
